...
class ScrapeMarketsCommandHandler:
    ...
    async def __scrape_market(
        ...
        await market_web_scrapper.navigate_to_home()
        logger.info(f"Navigated to {market_web_scrapper.url}")

        await market_web_scrapper.close_popups()
        logger.info("Closed popups")

        await market_web_scrapper.set_location(postal_code)
        logger.info(f"Location set to postal code {postal_code}")

        await market_web_scrapper.navigate_to_categories()
        logger.info("Navigated to categories")

        categories = await market_web_scrapper.get_categories()
        ...
        for category in categories:
            subcategories: list[
                Subcategory
            ] = await market_web_scrapper.get_subcategories(category)
            logger.info(
                f"Found {len(subcategories)} subcategories in category '{category}'"
            )

            for subcategory in subcategories:
                subcategory_products = await market_web_scrapper.scrape_subcategory(
                    subcategory
                )
